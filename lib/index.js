...
Nightwatch.prototype.setOptions = function(options) {
  ...
  this.api.launchUrl =
    this.options.launchUrl || this.options.launch_url || null;
  // backwords compatibility
  this.api.launch_url = this.api.launchUrl;

  if (
    this.options.globals &&
    typeof this.options.globals == "object" &&
    !this.options.persist_globals
  ) {
  ...
  if (screenshotsEnabled) {
    if (typeof screenshots.path == "undefined") {
      throw new Error(
        "Please specify the screenshots.path in nightwatch.json."
      );
    }
    this.options.screenshots.on_error =
      this.options.screenshots.on_error ||
      typeof this.options.screenshots.on_error == "undefined";
    this.api.screenshotsPath = this.api.options.screenshotsPath =
      screenshots.path;
  } else {
  ...
  this.options.start_session =
    this.options.start_session ||
    typeof this.options.start_session == "undefined";

  this.api.options.skip_testcases_on_fail =
    this.options.skip_testcases_on_fail ||
    (typeof this.options.skip_testcases_on_fail == "undefined" &&
      this.options.start_session); // off by default for unit tests

  this.api.options.log_screenshot_data =
  ...
  var key =
    this.options.accesKey || this.options.access_key || this.options.password;
  ...
  this.endSessionOnFail(
    typeof this.options.end_session_on_fail == "undefined" ||
      this.options.end_session_on_fail
  );
...
Nightwatch.prototype.setCapabilities = function() {
  ...
  for (var capability in Nightwatch.DEFAULT_CAPABILITIES) {
    this.desiredCapabilities[capability] =
      Nightwatch.DEFAULT_CAPABILITIES[capability];
  }
...
Nightwatch.prototype.start = function() {
  if (!this.sessionId && this.options.start_session) {
    this.once("selenium:session_create", this.start).startSession();
    return this;
  }
...
Nightwatch.prototype.terminate = function(deferred) {
  ...
  if (deferred) {
    this.queue
      .instance()
      .once("queue:started", this.terminateSession.bind(this));
  } else {
...
Nightwatch.prototype.getFailureMessage = function() {
  ...
  if (this.results.failed > 0) {
    failure_msg.push(
      Logger.colors.red(this.results.failed) + " assertions failed"
    );
  }
  ...
  if (this.options.output && this.options.start_session) {
    ...
    if (ok && this.results.passed > 0) {
      console.log(
        ...
        Logger.colors.green(this.results.passed) +
          " assertions passed. (" +
...
Nightwatch.prototype.handleException = function(err) {
  ...
  if (typeof err.actual != "undefined" && typeof err.expected != "undefined") {
    firstLine +=
      "\033[0;90m - expected " +
      Logger.colors.green('"' + err.expected + '"') +
      " \033[0;90mbut got: " +
      Logger.colors.red('"' + err.actual + '"');
  }
  ...
  var request = new HttpRequest(requestOptions)
    ...
    .on("error", function(result, response, screenshotContent) {
      ...
      if (screenshotContent && self.options.screenshots.on_error) {
        var fileNamePath = Utils.getScreenshotFileName(
          self.api.currentTest,
          true,
          self.options.screenshots.path
        );
...
Nightwatch.prototype.addError = function(message, logMessage) {
  ...
  if (this.api.currentTest) {
    currentTest =
      "[" +
      Utils.getTestSuiteName(this.api.currentTest.module) +
      " / " +
      this.api.currentTest.name +
      "]";
  } else {
    currentTest = "tests";
  }
...
Nightwatch.prototype.saveScreenshotToFile = function(fileName, content, cb) {
  ...
  var fail = function(err) {
    if (self.options.output) {
      console.log(
        Logger.colors.yellow("Couldn't save screenshot to "),
        fileName
      );
    }
...
Nightwatch.prototype.handleTestError = function(result) {
  ...
  if (result && result.status) {
    ...
    errorMessage =
      (errorCodes[result.status] && errorCodes[result.status].message) || "";
  }
...
Nightwatch.prototype.startSession = function() {
  ...
  request
    .on("success", function(data, response, isRedirect) {
      if (data && data.value.sessionId) {
        self.sessionId = self.api.sessionId = data.value.sessionId;
        if (data.value) {
          self.api.capabilities = data.value.capabilities;
        }
